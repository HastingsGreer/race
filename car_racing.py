...
class CarRacing(gym.Env):
    ...
    def render_road(self):
        if self.initialized < 3:
            ...
            gl.glBufferData(gl.GL_ARRAY_BUFFER_ARB, len(vertices) * 4, gl_vertices, gl.GL_STATIC_DRAW)
            
            
            gl_colors = (gl.GLfloat * len(colors))(*colors)
            gl.glBindBuffer(gl.GL_ARRAY_BUFFER_ARB, self.color_vbo_id)
            gl.glBufferData(gl.GL_ARRAY_BUFFER_ARB, len(colors) * 4, gl_colors, gl.GL_STATIC_DRAW)
